        ...
        self.replacements.iter().find_map(|replacement| {
            replacement.from.captures(&name.to_utf8()).map(|caps| {
                let vars: HashMap<String, String> =
                    caps.iter()
                        .enumerate()
                        .fold(HashMap::new(), |mut map, (index, cap)| {
                            map.insert(
                                index.to_string(),
                                cap.map_or_else(String::new, |c| c.as_str().to_string()),
                            );
                            map
                        });
...
impl RequestHandler for DomainConversionHandler {
    async fn handle_request<R: ResponseHandler>(
        ...
        let record = request.queries().first().and_then(|query| {
...
async fn create_server(
    config: Config,
) -> Result<ServerFuture<DomainConversionHandler>, Box<dyn std::error::Error>> {
...
#[command(
    about = "A simple DNS server that redirects queries based on regex replacements using cname records."
)]
struct Args {
    #[arg(
        short,
...
mod tests {
    ...
    fn setup_resolver(server_address: &str) -> Resolver<TokioConnectionProvider> {
        let name_server_config =
            NameServerConfig::new(SocketAddr::from_str(server_address).unwrap(), Protocol::Udp);
    ...
    async fn test_server(replacements: Vec<Replacement>, test_cases: Vec<(&str, &str)>) {
        ...
        let mut server = create_server(Config::new(address.clone(), replacements))
            .await
            .unwrap();
        ...
        // Perform lookups
        let lookup_results = stream
            .fold(vec![], |mut acc, (query, _)| async {
                let result = resolver
                    .lookup(Name::from_utf8(*query).unwrap(), RecordType::ANY)
                    .await;
                acc.push(result);
                acc
            })
        ...
        for ((query, expected), lookup_result) in test_cases.iter().zip(lookup_results.iter()) {
            ...
            let is_matching_cname = |record: &Record| match record.data() {
                RData::CNAME(cname) => cname.to_string() == *expected,
                _ => false,
            };

            assert!(
                result.records().iter().any(is_matching_cname),
                "Didn't find {} in the cnames",
                *expected
            );
        }
    }
    ...
    async fn test_no_match_returns_nxdomain() {
        let address = format!("127.0.0.1:{}", find_free_port().unwrap());
        let mut server = create_server(Config::new(
            address.clone(),
            vec![Replacement {
                from: regex::Regex::new(r"^(.*)\.mnh.?$").unwrap(),
        ...
        let lookup_result = resolver
            .lookup(Name::from_utf8("barry.net").unwrap(), RecordType::ANY)
            .await;
        ...
        match lookup_result {
            ...
            Err(e) => assert!(
                e.is_nx_domain(),
                "Expected NXDomain but got different error: {}",
                e
            ),
        };
    }
    ...
    async fn test_wrong_query_type_returns_nxdomain() {
        ...
        let lookup_result = resolver
            .lookup(Name::from_utf8("barry.net").unwrap(), RecordType::CSYNC)
            .await;
